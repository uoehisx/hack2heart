...
const BlankWrapper = styled.div<{ isOver: boolean }>`
  ...
  ${(p) =>
    p.isOver &&
    css`
      box-shadow: inset 4px 4px 12px rgba(0, 0, 0, 0.9), inset -4px -4px 12px #1a1a1a;
    `};
  display: flex;
  align-items: center;
  justify-content: center;
...
const DraggableCard: React.FC<DraggableCardProps> = ({ code, compact = false, onRemove, showDelete }) => {
  ...
  return (
    ...
    >
      {showDelete && !compact && (
        <CloseBtn big onClick={e => {e.stopPropagation();e.preventDefault();onRemove&&onRemove(code.id);}}>×</CloseBtn>
      )}
      <strong style={{ fontSize: compact ? 14 : 16 }}>{code.title}</strong>
      {!compact && (
        <span style={{ marginTop: "auto", fontSize: 12, color: "#888" }}>{code.length} chars</span>
      )}
    </CardWrapper>
  );
};
  ...
  return (
    <Wrapper>
      ...
      >
        ...
        <BlanksContainer>
          {blanks.map((blank) => (
            <DroppableBlank key={blank.id} blank={blank} onRemove={removeFromBlank} />
          ))}
        ...
        <div id="codes" style={{ width: "100%" }}>
          <CardGrid>
            {codes.map((code) => (
              <DraggableCard key={code.id} code={code} onRemove={deleteCodeCard} showDelete />
            ))}
          </CardGrid>
        </div>
